<x-app-layout>
    @section('title', 'Store Furniture')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @lang('crud.store_furniture.index_title')
        </h2>
    </x-slot>
                ...
                <div class="block w-full overflow-auto scrolling-touch">
                    <table class="w-full max-w-full mb-4 bg-transparent">
                        ...
                        <tbody class="text-gray-600">
                            @forelse($storeFurnitures as $storeFurniture)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-left">
                                    {{ $storeFurniture->name ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                    {{ $storeFurniture->description ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-right">
                                    {{ $storeFurniture->price ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                ...
                                >
                                    <div
                                        ...
                                        "
                                    >
                                        @can('update', $storeFurniture)
                                        <a
                                            href="{{ route('store-furnitures.edit', $storeFurniture) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('view', $storeFurniture)
                                        <a
                                            href="{{ route('store-furnitures.show', $storeFurniture) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('delete', $storeFurniture)
                                        <form
                                            action="{{ route('store-furnitures.destroy', $storeFurniture) }}"
                                            method="POST"
