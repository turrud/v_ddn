<x-app-layout>
    @section('title', 'Store3d Furniture')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @lang('crud.store3d_furniture.index_title')
        </h2>
    </x-slot>
                ...
                <div class="block w-full overflow-auto scrolling-touch">
                    <table class="w-full max-w-full mb-4 bg-transparent">
                        ...
                        <tbody class="text-gray-600">
                            @forelse($store3dFurnitures as $store3dFurniture)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-left">
                                    {{ $store3dFurniture->name ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                    {{ $store3dFurniture->description ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-right">
                                    {{ $store3dFurniture->price ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                ...
                                >
                                    <div
                                        ...
                                        "
                                    >
                                        @can('update', $store3dFurniture)
                                        <a
                                            href="{{ route('store3d-furnitures.edit', $store3dFurniture) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('view', $store3dFurniture)
                                        <a
                                            href="{{ route('store3d-furnitures.show', $store3dFurniture) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        $store3dFurniture)
                                        <form
                                            action="{{ route('store3d-furnitures.destroy', $store3dFurniture) }}"
                                            method="POST"
